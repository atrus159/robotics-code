...
void executeGeneral(){
				...
				while(TSreadState(msensor_S2_2)==0)
	{
	  motor[leftMotor]=20;
  	  ...
  	  nMotorEncoder[leftMotor]=0;
      nMotorEncoder[rightMotor]=0;
      while(nMotorEncoder[rightMotor]>-0.5*1120){
    	motor[leftMotor]=-30;
			motor[rightMotor]=-10;
  }
  counter=0;
      	 compassOffset=HTMCreadHeading(msensor_S2_3);
     while(getAngle(HTMCreadHeading(msensor_S2_3)-compassOffset)<60 || counter<3){
  	  motor[leftMotor]=10;
			motor[rightMotor]=-10;
						if(getAngle(HTMCreadHeading(msensor_S2_3)-compassOffset)>=60){
		counter++;		
		}
		else{
			counter=0;
